...
class PromptLibraryV2 {
  ...
  updatePrompt(
    ...
    const existingPrompt = this.prompts.find(p => p.id === id)
    if (!existingPrompt) {
      return false
    }
    ...
    this.prompts = this.prompts.map(p => (p.id === id ? updatedPrompt : p))
    this.saveLocalPrompts()

    if (isFirebaseEnabled && currentUser) {
      this.syncPromptToCloud(updatedPrompt)
    }
  ...
  mergeCloudData(cloudPrompts, cloudFolders) {
    const promptMap = new Map(this.prompts.map(prompt => [prompt.id, prompt]))

    cloudPrompts.forEach(cloudPrompt => {
      const existingPrompt = promptMap.get(cloudPrompt.id)
      if (!existingPrompt) {
        promptMap.set(cloudPrompt.id, cloudPrompt)
        return
      }

      const localDate = new Date(existingPrompt.dateModified)
  ...
  auth.onAuthStateChanged(async user => {
    ...
    if (user) {
      ...
      if (localPromptsWithoutUser.length > 0) {
        if (
          confirm(
            `Would you like to sync your ${localPromptsWithoutUser.length} local prompts to the cloud?`
          )
        ) {
          try {
...
function handleFolderFormSubmit(e) {
  ...
  const color = document.querySelector(
    'input[name="folderColor"]:checked'
  ).value
...
function createFolderElement(folder) {
  ...
  button.className =
    'folder-item' + (library.currentFolderId === folder.id ? ' active' : '')
...
function updateCounts() {
  ...
  if (favCount)
    favCount.textContent = library.prompts.filter(p => p.starred).length
}
...
function toggleStar(id) {
  ...
  renderPrompts()  // Just re-render prompts, don't reset folder state
  updateCounts()   // Update counts separately
}
...
function calculateStats() {
  ...
  document.getElementById('totalUsesStatValue').textContent = prompts.reduce(
    (sum, p) => sum + p.useCount,
    0
  )
  document.getElementById('favoriteCountStatValue').textContent =
    prompts.filter(p => p.starred).length
  document.getElementById('folderCountStatValue').textContent =
    library.getAllFolders().length
  ...
  document.getElementById('categoryBreakdownStats').innerHTML =
    categoryHtml ||
    '<p style="color: var(--text-secondary); text-align: center; padding: 1rem;">No prompts yet</p>'
}
